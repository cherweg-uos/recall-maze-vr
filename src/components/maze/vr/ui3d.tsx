    ...
    <group>
      ...
      {ticks.map((f) => (
        <mesh key={f} position={[-width / 2 + f * width, 0, 0.015]} raycast={() => null}>
          ...
          <meshBasicMaterial color={UI.inkSoft} toneMapped={false} />
        </mesh>
      ))}
...
export function SliderRow({
  ...
  return (
    <group {...props}>
      ...
      <Label position={[-0.75, -0.08, 0.02]} anchorX="left" size={0.055} color={UI.inkSoft}>
        {label}
      </Label>
      <Label position={[0.75, -0.08, 0.02]} anchorX="right" size={0.055} color={UI.ink}>
        {valueText}
      </Label>
    </group>
  );
